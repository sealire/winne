package org.winne.base.framework.mybatis;
...
public class RenameExampleMethodPlugin extends PluginAdapter {
    ...
    public boolean clientCountByExampleMethodGenerated(Method method, Interface interfaze, IntrospectedTable introspectedTable) {
        method.setName(method.getName().replaceAll("Example", this.replaceString));
    ...
    public boolean sqlMapCountByExampleElementGenerated(XmlElement element, IntrospectedTable introspectedTable) {
        ...
        for (int i = 0; i < list.size(); i++) {
            Element old = (Element) list.get(i);
    ...
    private void replaceIdAttribute(XmlElement element) {
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            if ("id".equals(old.getName())) {
                Attribute repace = new Attribute("id", old.getValue().replaceAll("Example", this.replaceString));
                list.set(i, repace);
                break;
            }
        }
